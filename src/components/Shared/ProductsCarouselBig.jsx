...
import ProductDisplayBig from "./ProductDisplayBig"
...
export default function ProductsCarouselBig() {
  return (
    <>
      <Swiper
        slidesPerView={1}
      ...
      >
        {products.map((product) => (
            <SwiperSlide className='min-h-fit' key={product.id}>
              <ProductDisplayBig id={product.id} name={product.name} price={product.price} category={product.category} img={product.img} />
            </SwiperSlide>
        ))}
      </Swiper>
    </>
